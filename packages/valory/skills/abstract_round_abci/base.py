...
# Raised to 300s to ride out the slow redeem / update-bets phase: on a safe
# with history this can run up to ~2 min of consecutive subgraph/RPC work,
# and the 60s default made the agent trip the deadline on every period and
# force a Tendermint reset (which could deadlock the agent mid-behaviour).
BLOCKS_STALL_TOLERANCE = 300
